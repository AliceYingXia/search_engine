...
## Appendix — Demo 1 Production Demo

`demo_1` is the streamlined production-style demo built from the evaluation
work above. It keeps the overall retrieval ideas, but packages them into a
simple three-stage pipeline with OpenSearch as the serving backend.
...
### Stage 1 — Process Raw JSON

Script:
- `demo_1/01_process_json/process_recipe_json_files.py`

Input:
- raw recipe JSON files

Output:
- `demo_1/01_process_json/processed/recipes.parquet`

The processor converts nested recipe payloads into normalized searchable
records with:
- `text_no_comments`
- `connectors`
- `actions`
- `input_fields`
- `datapill_fields`
- metadata such as `recipe_uid`, `flow_id`, `version_no`, and `step_count`

### Stage 2 — Ingest to OpenSearch

Scripts:
- `demo_1/02_ingest_opensearch/create_indices.py`
- `demo_1/02_ingest_opensearch/index_processed_recipes.py`
- `demo_1/02_ingest_opensearch/build_embeddings.py`

This stage:
1. creates the lexical `recipes` index
2. indexes processed recipe records
3. builds dense vectors for the same recipes

The demo uses only one embedding model:
- `Qwen/Qwen3-Embedding-8B-full+instruct`

### Stage 3 — Retrieve on Live Query

Script:
- `demo_1/03_query/search_recipes.py`

The serving path always uses weighted Reciprocal Rank Fusion (RRF) across:
- full-text search over OpenSearch
- dense vector search over OpenSearch

This demo no longer supports route-only behavior. It is fusion-only.

### Query-Signal Weighting

The query is first classified into one of three heuristic buckets:
...
```text
score(doc) += weight / (60 + rank)
```

Each leg retrieves `k * 3` candidates before fusion, then the final list is
truncated to `top_k`.

### Robustness Improvements in Demo 1

The demo code has been hardened with small, explicit safeguards:

- input validation for JSON processing and parquet ingestion
- required-column checks before indexing
- friendlier OpenSearch connection errors
- friendlier Baseten embedding errors
- batch-level error messages during embedding generation
- clean CLI failure messages instead of raw stack traces

In addition, `search_recipes.py` supports an optional demo-safe fallback:

```bash
python demo_1/03_query/search_recipes.py \
  --query "which automations handle employee onboarding" \
  --allow-fts-fallback
```

If dense embedding fails and this flag is enabled, the script will degrade to
FTS-only. If the query would not normally run FTS, it will still issue a
last-resort FTS search rather than exiting immediately.

### Validation Outside the Demo Folder

To keep test and evaluation code separate from the demo package itself:

- unit tests live at `tests/test_demo_1.py`
- retrieval evaluation for Category 1/2/3 lives at
  `tests/evaluate_demo_1_retrieval_strategy.py`

Unit test:

```bash
python3 -m unittest tests.test_demo_1
```

Category evaluation:

```bash
venv/bin/python tests/evaluate_demo_1_retrieval_strategy.py --k 5
```

This keeps `demo_1` focused on the runtime pipeline, while still allowing the
demo retrieval strategy to be tested and benchmarked from the repo root.
