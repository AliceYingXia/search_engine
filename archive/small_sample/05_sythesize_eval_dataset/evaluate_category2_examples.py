"""
evaluate_category2_examples.py
================================

Uses GPT-5.2 to independently evaluate the Category 2 example Excel files
produced by category_2_eval_dataset.py.

What this script does
---------------------
For each example (category2_example_1.xlsx ... category2_example_50.xlsx):

  1. Query quality assessment
     GPT-5.2 rates the query on two dimensions:
       - Clarity     : Is the query unambiguous and well-formed?
       - Specificity : Is it appropriately specific for a Category 2 query
                       (names systems, trigger, and outcome)?
...
Output
------
  05_sythesize_eval_dataset/category2_examples/evaluation_results.xlsx
...
EXAMPLE_FILES = sorted(EXAMPLES_DIR.glob("category2_example_*.xlsx"))
...
concrete systems involved, the trigger, and the result. A good Category 2 query:
  - Is unambiguous and clearly written.
  - Names the specific systems, trigger, and outcome \
(e.g. "Which recipe creates a NetSuite vendor bill when a PO is approved in Coupa?").
  - Is narrow enough that only a small number of recipes would match exactly.
...
A search query has been issued and a set of candidate recipes have been labelled:
  strong — the recipe is highly likely to be the exact automation described by \
the query; the trigger, action, and systems match closely
  weak   — the recipe performs a similar or adjacent action but differs in \
trigger, system, or outcome
...
def main():
    if not EXAMPLE_FILES:
        raise FileNotFoundError(
            f"No category2_example_*.xlsx files found in {EXAMPLES_DIR}. "
            "Run category_2_eval_dataset.py first."
        )
