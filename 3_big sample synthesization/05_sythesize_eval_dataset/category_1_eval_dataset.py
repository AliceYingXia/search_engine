"""
category_1_eval_dataset.py
==========================

Phase 3 of the Category 1 evaluation dataset pipeline.
Run this AFTER build_eval_category1_relevance.py.

What this script does
---------------------
1. Loads eval_category1.csv (Phase 2 output).
...
3. Produces two outputs:
     a. eval_category1_summary.csv  — one row per query with strong/weak lists
        from each model separately.
     b. eval_category1_detail.csv   — every remaining row enriched with
...
Output
------
  05_sythesize_eval_dataset/category1_eval_dataset.csv
  05_sythesize_eval_dataset/eval_category1_detail.csv
    ...
    Detail columns:
      all columns from eval_category1.csv  +  recipe_uid  +  recipe_summary
...
INPUT_PATH  = BASE_DIR / "eval_category1.csv"

SUMMARY_PATH = BASE_DIR / "category1_eval_dataset.csv"
DETAIL_PATH  = BASE_DIR / "eval_category1_detail.csv"
...
def main():
    ...
    with queries_txt_path.open("w", encoding="utf-8") as f:
        f.write(f"Category 1 Evaluation Queries  ({len(summary)} total)\n")
        f.write("=" * 70 + "\n\n")
        current_author = None
    ...
    for i, query_id in enumerate(sample_ids, 1):
        ...
        out_path = EXAMPLES_DIR / f"category1_example_{i}.xlsx"
    ...
    for col, filename, title in [
        ("strong_count", "category1_eval_strong_count", "Distribution of Strong List Counts per Query"),
        ("weak_count",   "category1_eval_weak_count",   "Distribution of Weak List Counts per Query"),
    ]:
        fig, ax = plt.subplots()
