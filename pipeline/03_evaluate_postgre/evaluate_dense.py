...
-----
    ...
    python pipeline/03_evaluate_postgre/evaluate_pgvector.py --model text-embedding-3-small
    python pipeline/03_evaluate_postgre/evaluate_pgvector.py --model text-embedding-3-large --k 10
    python pipeline/03_evaluate_postgre/evaluate_pgvector.py --model "BAAI/bge-m3" "Qwen/Qwen3-Embedding-0.6B"
    python pipeline/03_evaluate_postgre/evaluate_pgvector.py --model "Qwen/Qwen3-Embedding-0.6B" "Qwen/Qwen3-Embedding-0.6B+instruct"
...
class PgVectorEvaluator:
    ...
    def _search(self, embedding: list[float]) -> list[str]:
        ...
        if self.search_mode == "exact":
            self.cur.execute("SET LOCAL enable_indexscan = off")
            self.cur.execute("SET LOCAL enable_bitmapscan = off")
            self.cur.execute("SET LOCAL enable_seqscan = on")
        elif self.search_mode == "hnsw":
            self.cur.execute("SET LOCAL enable_seqscan = off")
            self.cur.execute("SET LOCAL hnsw.ef_search = %s", (self.hnsw_ef_search,))
        else:
            raise ValueError(f"Unsupported search mode: {self.search_mode!r}")
    ...
    def run(
        ...
        print(
            ...
            f"  |  search={self.search_mode}  |  k={k}"
        )
        print(
            f"Storage: pg_type={cfg.pg_type}  |  raw_dim={cfg.dimension or 'query-only'}"
            f"{store_dim_note}"
        )
        if self.search_mode == "hnsw":
            print(f"HNSW ef_search={self.hnsw_ef_search}")
        print(f"{'=' * 60}")
...
def main():
    ...
    parser.add_argument(
        ...
        default="exact",
        help=(
            "Search execution strategy. 'exact' forces a sequential scan for exact kNN "
            "over the stored vectors; 'hnsw' forces index usage and sets hnsw.ef_search."
        ),
    )
    parser.add_argument(
        "--hnsw-ef-search",
        type=int,
        default=80,
        help="hnsw.ef_search to use when --search-mode hnsw (default: 80)",
    )
    ...
    for model_name in args.model:
        ...
        out_path  = BASE_DIR / f"eval_results_{safe_name}_{args.search_mode}_k{k}.csv"
