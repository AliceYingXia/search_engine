"""
generate_golden_answers.py

Build golden answers for step-level search queries.

For each (query, recipe) pair:
  - Call GPT once (temperature=0, deterministic) to identify relevant steps

All 629 recipes are evaluated for every query (no BM25/retrieval filter).

Run:
    python generate_golden_answers.py

Output:
    pipeline/02_generate_descriptions/golden_answers.parquet
...
OUTPUT_PARQUET = Path(__file__).parent / "golden_answers.parquet"
...
async def call_gpt(semaphore: asyncio.Semaphore, query: str, search_text: str) -> list[str]:
    async with semaphore:
        ...
        while attempt < 5:
            try:
                ...
                attempt += 1
                log.warning(f"GPT error (attempt {attempt}): {e}")
                if attempt < 5:
...
async def main():
    ...
    gpt_sem = asyncio.Semaphore(MAX_CONCURRENT)
    ...
    for i, (query, query_type) in enumerate(all_queries):
        ...
        tasks = [
            call_gpt(gpt_sem, query, row["search_text"])
