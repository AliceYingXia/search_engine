...
This ensures the evaluation covers the breadth of the recipe corpus, not just the most common patterns. The pool is deliberately kept to ~115 recipes so that Phase 4 can exhaustively score every (query, recipe) pair using LLMs — ensuring no relevant recipe is overlooked in the ground truth. Retrieval evaluation also searches against this same recipe pool.
...
Exact-2000 uses the dedicated truncated `vector(2000)` table.
...
| Exact scan | forced seq scan | 2,000 (vector) | **0.4322** | **0.3817** | **0.56** | 12.88 ms |
...
| Exact scan | forced seq scan | 2,000 (vector) | **0.9100** | **0.7983** | **0.92** | 13.07 ms |
...
| Exact scan | forced seq scan | 2,000 (vector) | 0.7327 | **0.5935** | **0.7959** | 11.65 ms |
...
- **The first 2,000 dimensions remain surprisingly strong.** Compared with full 4,096 exact search, `2000 exact` slightly improves Category 1 and Category 2 MRR, while only slightly reducing Category 3 Recall@5 (0.7388 → 0.7327).
...
- **The main practical takeaway is that `halfvec(4000)` is the safest pgvector production choice for this workload.** It preserves full-model quality while enabling HNSW indexing. The `2000`-dim variant is also a credible compression option, but it introduces a small recall tradeoff on Category 3.
