...
Creates the recipes index and one or more embedding indices in OpenSearch.
...
-----
    ...
    # Only create the index needed by text-embedding-3-small
    python pipeline/04_evaluate_opensearch/create_opensearch_indices.py \
...
To drop individual indices manually:
    DELETE /recipes
    DELETE /embeddings_text_embedding_3_small
"""
...
class IndexManager:
    def __init__(self, client, recreate: bool = False, models: list[str] | None = None):
        self.client   = client
        self.recreate = recreate
        self.models   = models or INGESTABLE_MODELS
    ...
    def setup(self) -> None:
        ...
        for name, dim in resolve_embedding_indices(self.models):
...
def resolve_embedding_indices(models: list[str]) -> list[tuple[str, int]]:
    """Return unique (index_name, dimension) pairs for the requested models."""
    indices: list[tuple[str, int]] = []
    seen: set[str] = set()

    for model_name in models:
        cfg = MODEL_REGISTRY[model_name]
        if cfg.dimension is None or cfg.table in seen:
            continue
        dimension = cfg.store_dim or cfg.dimension
        indices.append((cfg.table, dimension))
        seen.add(cfg.table)
...
def main():
    ...
    parser.add_argument(
        ...
        default=["all"],
        help="Embedding model(s) whose indices should exist. Pass one, several, or 'all' (default: all).",
    )
    parser.add_argument(
        "--recreate",
        action="store_true",
    ...
    manager = IndexManager(get_client(), recreate=args.recreate, models=models)
    manager.setup()
